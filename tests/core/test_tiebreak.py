...
class TestTiebreakInit:
    ...
    def test_init_default_score(self):
        tb = Tiebreak(playerServing=1, isSuper=False, matchFormat=DEFAULT_FORMAT)
    ...
    def test_init_player2_serves(self):
        tb = Tiebreak(playerServing=2, isSuper=False, matchFormat=DEFAULT_FORMAT)
        assert tb.servesNext == 2

    def test_init_super_tiebreak(self):
        tb = Tiebreak(playerServing=1, isSuper=True, matchFormat=DEFAULT_FORMAT)
        assert tb.score._isSuper is True
        assert tb.score.pointsToWin == 10

    def test_init_with_custom_score(self):
        init_score = TiebreakScore(3, 2, isSuper=False, matchFormat=DEFAULT_FORMAT)
        tb = Tiebreak(playerServing=1, isSuper=False, initScore=init_score)
        assert tb.score.asPoints(1) == (3, 2)

    def test_init_with_custom_super_score(self):
        init_score = TiebreakScore(5, 4, isSuper=True, matchFormat=DEFAULT_FORMAT)
        tb = Tiebreak(playerServing=1, initScore=init_score, isSuper=True)
        assert tb.score.asPoints(1) == (5, 4)
        assert tb.score._isSuper is True

    def test_init_invalid_server(self):
        with pytest.raises(ValueError):
            Tiebreak(playerServing=0, isSuper=False, matchFormat=DEFAULT_FORMAT)
        with pytest.raises(ValueError):
            Tiebreak(playerServing=3, isSuper=False, matchFormat=DEFAULT_FORMAT)

    def test_init_invalid_score_type(self):
        with pytest.raises(ValueError):
            Tiebreak(playerServing=1, isSuper=False, initScore="invalid", matchFormat=DEFAULT_FORMAT)

    def test_init_invalid_isSuper_type(self):
        with pytest.raises(ValueError):
            Tiebreak(playerServing=1, isSuper="yes", matchFormat=DEFAULT_FORMAT)

    def test_init_mismatched_isSuper(self):
        # initScore is regular tiebreak, but isSuper=True
        init_score = TiebreakScore(3, 2, isSuper=False, matchFormat=DEFAULT_FORMAT)
        with pytest.raises(ValueError):
            Tiebreak(playerServing=1, initScore=init_score, isSuper=True)

        # initScore is super tiebreak, but isSuper=False
        init_score = TiebreakScore(3, 2, isSuper=True, matchFormat=DEFAULT_FORMAT)
        with pytest.raises(ValueError):
...
class TestTiebreakRepr:
    ...
    def test_repr_contains_server(self):
        tb = Tiebreak(playerServing=2, isSuper=False, matchFormat=DEFAULT_FORMAT)
        assert "playerServing=2" in repr(tb)
...
class TestTiebreakWithInitialScore:
    ...
    def test_start_at_5_4(self):
        init_score = TiebreakScore(5, 4, isSuper=False, matchFormat=DEFAULT_FORMAT)
        tb = Tiebreak(playerServing=1, isSuper=False, initScore=init_score)
    ...
    def test_start_at_match_point(self):
        init_score = TiebreakScore(6, 4, isSuper=False, matchFormat=DEFAULT_FORMAT)
        tb = Tiebreak(playerServing=2, isSuper=False, initScore=init_score)
    ...
    def test_start_near_end_super(self):
        init_score = TiebreakScore(9, 8, isSuper=True, matchFormat=DEFAULT_FORMAT)
        tb = Tiebreak(playerServing=1, isSuper=True, initScore=init_score)
