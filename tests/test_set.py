...
class TestSetInit:
    ...
    def test_init_default_score(self):
        s = Set(playerToServe=1, isFinalSet=False, matchFormat=DEFAULT_FORMAT)
    ...
    def test_init_player2_serves(self):
        s = Set(playerToServe=2, isFinalSet=False, matchFormat=DEFAULT_FORMAT)
        assert s.servesNext == 2

    def test_init_final_set(self):
        s = Set(playerToServe=1, isFinalSet=True, matchFormat=DEFAULT_FORMAT)
    ...
    def test_init_with_custom_score(self):
        ...
        s = Set(playerToServe=1, isFinalSet=False, initScore=init_score)
        assert s.score.games(1) == (3, 2)

    def test_init_invalid_server(self):
        with pytest.raises(ValueError):
            Set(playerToServe=0, isFinalSet=False, matchFormat=DEFAULT_FORMAT)
        with pytest.raises(ValueError):
            Set(playerToServe=3, isFinalSet=False, matchFormat=DEFAULT_FORMAT)

    def test_init_invalid_isFinalSet_type(self):
        with pytest.raises(ValueError):
            Set(playerToServe=1, isFinalSet="yes", matchFormat=DEFAULT_FORMAT)

    def test_init_invalid_score_type(self):
        with pytest.raises(ValueError):
            Set(playerToServe=1, isFinalSet=False, initScore="invalid", matchFormat=DEFAULT_FORMAT)

    def test_init_missing_matchFormat(self):
        with pytest.raises(ValueError):
            Set(playerToServe=1, isFinalSet=False)

    def test_init_mismatched_isFinalSet(self):
        init_score = SetScore(3, 2, isFinalSet=True, matchFormat=DEFAULT_FORMAT)
        with pytest.raises(ValueError):
            Set(playerToServe=1, isFinalSet=False, initScore=init_score)

    def test_init_mismatched_matchFormat(self):
        init_score = SetScore(3, 2, isFinalSet=False, matchFormat=DEFAULT_FORMAT)
        with pytest.raises(ValueError):
            Set(playerToServe=1, isFinalSet=False, initScore=init_score, matchFormat=NO_AD_FORMAT)
...
class TestSetRepr:
    ...
    def test_repr_contains_server(self):
        s = Set(playerToServe=2, isFinalSet=False, matchFormat=DEFAULT_FORMAT)
        assert "playerToServe=2" in repr(s)
...
class TestSetNoAdRule:
    ...
    def test_no_ad_game_ends_at_deuce(self):
        s = Set(playerToServe=1, isFinalSet=False, matchFormat=NO_AD_FORMAT)
        # Get to deuce (40-40)
