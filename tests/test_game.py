...
class TestGameInit:
    ...
    def test_init_with_no_ad_rule(self):
        init_score = GameScore(3, 3, noAdRule=True)
        game = Game(playerToServe=1, initScore=init_score)
        game.recordPoint(1)
        assert game.isOver
...
class TestGameNoAdRule:
    ...
    def test_no_ad_standard_win(self):
        init_score = GameScore(0, 0, noAdRule=True)
        game = Game(playerToServe=1, initScore=init_score)
